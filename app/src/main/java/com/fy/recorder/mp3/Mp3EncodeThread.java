...
import com.fy.recorder.RecordService;
...
public class Mp3EncodeThread extends Thread {
    ...
    public Mp3EncodeThread(File file, int bufferSize) {
        ...
        L.d(TAG, "in_sampleRate:%s，getChannelCount:%s ，out_sampleRate：%s 位宽： %s," + sampleRate + currentConfig.getChannelCount() + sampleRate + currentConfig.getRealEncoding());
    ...
    public void run() {
        ...
        while (start) {
            ...
            L.v(TAG, "处理数据：%s" + (next == null ? "null" : next.getReadSize()));
            lameData(next);
        }
    }
    ...
    private void lameData(ChangeBuffer changeBuffer) {
        ...
        if (readSize > 0) {
            ...
            if (encodedSize < 0) {
                L.e(TAG, "Lame encoded size: " + encodedSize);
            }
            try {
                os.write(mp3Buffer, 0, encodedSize);
            } catch (IOException e) {
                L.e(TAG, "Unable to write to file");
            }
        }
    }
