...
import com.gcstorage.parkinggather.widget.datepicker.entities.DPInfo;
...
public class MonthView extends View {
    ...
    private boolean isNewEvent,
            ...
            isDescribeDisplay = false;

    private boolean canSrollHor = true,
            canSrcollVer = true;
    ...
    private boolean isBottomTextShow = false;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                } else if (canSrollHor && mSlideMode == SlideMode.HOR) {
    ...
    protected void onSizeChanged(int w, int h, int oldW, int oldH) {
        ...
        int cellW = (int) (w / 7F);
        int cellH4 = (int) (h / 4F - 0.5);
        int cellH5 = (int) (h / 5F - 0.5);
        int cellH6 = (int) (h / 6F - 0.5);
        ...
        sizeTextGregorian = MeasureUtil.dp2px(getContext(),14F);
    ...
    void setShowAnima(boolean showAnima) {
        isShowAnima = showAnima;
    }

    void setBottomTextShow(boolean bottomTextShow) {
        isBottomTextShow = bottomTextShow;
    }
    ...
    private void draw(Canvas canvas, Rect rect, DPInfo info) {
        ...
        drawGregorian(canvas, rect, info, info.isWeekend);
        if (isFestivalDisplay){
            drawFestival(canvas, rect, info.strF, info.isFestival);
        }
        if(isBottomTextShow){
            if (null != mDPDecor) {
                mDPDecor.drawBottomText(canvas, rect, mPaint,
    ...
    private void drawBGToday(Canvas canvas, Rect rect) {
        ...
        canvas.drawCircle(rect.centerX(), rect.centerY(), MeasureUtil.dp2px(getContext(),19f), mPaint);
    }
    ...
    private void drawGregorian(Canvas canvas, Rect rect, DPInfo info, boolean isWeekend) {
        mPaint.setTextSize(sizeTextGregorian);
        //        if (isWeekend) {
    ...
    void setDateWithChange(int year, int month) {
        centerYear = year;
        centerMonth = month;
        buildRegion();
        computeDate();
    ...
    void setCanSrollHor(boolean canSrollHor){
        this.canSrollHor = canSrollHor;
    }

    void setCanSrcollVer(boolean canSrcollVer){
        this.canSrcollVer = canSrcollVer;
    }
    ...
    void setDescribeDisplay(boolean isDescribeDisplay) {
        this.isDescribeDisplay = isDescribeDisplay;
    }
            ...
            for (int j = 0; j < tmp[i].length; j++) {
                ...
                if (region.contains(x, y)) {
                    ...
                    if (mDPMode == DPMode.SINGLE) {
                        ...
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB && isShowAnima) {
    ...
    public void setHeightScale(float month_view_height_scale) {
        this.heightScale = month_view_height_scale;
    }
