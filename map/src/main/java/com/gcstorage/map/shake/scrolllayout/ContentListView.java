...
public class ContentListView extends ListView {
     ...
     */
    public void removeOnScrollListener(final AbsListView.OnScrollListener listener) {
        throwIfNotOnMainThread();
    ...
    private class CompositeScrollListener implements AbsListView.OnScrollListener {
        private final List<AbsListView.OnScrollListener> scrollListenerList = new
                ArrayList<AbsListView.OnScrollListener>();

        public void addOnScrollListener(AbsListView.OnScrollListener listener) {
            if (listener == null) {
                return;
            }
            for (AbsListView.OnScrollListener scrollListener : scrollListenerList) {
        ...
        public void removeOnScrollListener(AbsListView.OnScrollListener listener) {
            ...
            while (iterator.hasNext()) {
                AbsListView.OnScrollListener scrollListener = iterator.next();
                if (listener == scrollListener) {
                    iterator.remove();
                    return;
                }
        ...
        @Override
        public void onScrollStateChanged(AbsListView view, int scrollState) {
            List<AbsListView.OnScrollListener> listeners = new ArrayList<AbsListView.OnScrollListener>(scrollListenerList);
